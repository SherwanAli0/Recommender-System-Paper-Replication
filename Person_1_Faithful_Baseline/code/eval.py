"""
eval.py - Person 1 / Stream P1 (Faithful Baseline)
Protocol A: warm-user 10-fold CV for FUS and CF.

FUS results reused from Person 2 (identical algorithm, same dataset, same folds).
This script runs only the CF sweep (unique to Person 1) then generates the
...
# (Implementation/Person_1_Faithful_Baseline/code/ → ../../../ml-100k)
DATA_DIR     = os.path.join(_HERE, "..", "..", "..", "ml-100k")
RESULTS_DIR  = os.path.join(_HERE, "..", "results")
FIGS_DIR     = os.path.join(RESULTS_DIR, "figs")
RATINGS_PATH = os.path.join(DATA_DIR, "u.data")

# Person 2 FUS CSV - reused instead of rerunning the identical FUS algorithm
# (Person 2 writes to Person_2_Faithful_FUS/results/protocol_A/results_FUS_A_warm.csv
# under the same repo)
P2_FUS_CSV = os.path.join(_HERE, "..", "..", "Person_2_Faithful_FUS",
...
def main():
    t0 = time.time()
    print("=== Protocol A -- Person 1 Faithful Baseline ===")

    # ------------------------------------------------------------------
    # Step 1: Reuse Person 2 FUS CSV (same algorithm, no need to rerun)
    ...
    if not os.path.exists(P2_FUS_CSV):
        print("ERROR: Person 2 FUS CSV not found at:")
        print("  " + P2_FUS_CSV)
        print("Run Person_2_Faithful_FUS/code/eval_fus.py first.")
        return
    shutil.copy2(P2_FUS_CSV, p1_fus_csv)
    print("Copied FUS CSV from Person 2 ->")
    print("  " + p1_fus_csv)
    ...
    # ------------------------------------------------------------------
    # Step 3: CF (Pearson KNN) sweep -- unique Person 1 contribution
